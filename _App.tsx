...
function buildMixed(level: MixedLevel): Question[] {
  ...
  while (questions.length < 10) {
    ...
    const right = tables[Math.floor(Math.random() * tables.length)];

    // Skip if both are 0
    if (left === 0 && right === 0) continue;

    // Normalize the key so 6x7 and 7x6 are treated as the same
...
function buildMemory(level: MemoryLevel): MemoryCard[] {
  ...
  for (const a of tables) {
    for (let b = 1; b <= 10; b++) {
      const result = a * b;
      if (!resultMap.has(result)) resultMap.set(result, []);
      resultMap.get(result)!.push([a, b]);
    }
  }
...
export default function App() {
  ...
  const seqScore = useMemo(
    () => seqStatuses.filter((s) => s === "correct").length,
    [seqStatuses]
  );
            ...
            <View style={styles.gameArea}>
              ...
              {!seqDone ? (
                ...
                <View style={styles.resultCard}>
                  ...
                  <StarRating correctCount={seqScore} />
...
const styles = StyleSheet.create({
  ...
  levelGrid: { flexDirection: "row", flexWrap: "wrap", justifyContent: "center", gap: 10, direction: "ltr" },
  levelTile: {
    width: 96,
    height: 96,
    borderRadius: 16,
